    ...
    for col in range(1,n):
        ...
        for row_down in range(1,n):
            ...
            if DQ[row_down][col] != None:
                if DQ[row_down-1][col] != None and DQ[row_down][col-1] != None and (quo_vadis[row_down][col-1] != [0,0,0] or quo_vadis[row_down-1][col] != [0,0,0]): # Istnieje góra i lewo
                    if DQ[row_down-1][col] == DQ[row_down][col-1]: # Są takie same
                        DQ[row_down][col] = DQ[row_down][col-1] + 1
                        quo_vadis[row_down][col] = [1,1,0]
                    else: # Są różne
                        DQ[row_down][col] = max(
