...
def highway(A):
    ...
    # Wynik
    print(curren_max - curren_min)


def usuwanie_kraw(is_edge_occupied, vertex_edges_connected, is_vertes_ocuppied, Edges_sorted, min_edge, max_edge):
    """Usuwa minimalną krawędź i dodaje inne, jeśli graf nie jest spójny."""
    import heapq

    i = 0
    while i < len(is_edge_occupied):
        if is_edge_occupied[i] and Edges_sorted[i][2] == min_edge:
            is_edge_occupied[i] = False
            v1, v2 = Edges_sorted[i][0], Edges_sorted[i][1]

            vertex_edges_connected[v1].remove(i)
            vertex_edges_connected[v2].remove(i)
            break
        i += 1

    # Sprawdzenie, czy graf jest nadal spójny
    if not are_all_connected(is_vertes_ocuppied):
        heap = []

        # Dodawanie krawędzi większych od aktualnego minimum
        for edge in Edges_sorted:
            if edge[2] > min_edge:
                heapq.heappush(heap, edge)

        while not are_all_connected(is_vertes_ocuppied):
            new_edge = heapq.heappop(heap)
            v1, v2, dist, ids = new_edge
            if not is_vertes_ocuppied[v1] or not is_vertes_ocuppied[v2]:
                process_edge(v1, v2, dist, ids, is_vertes_ocuppied, vertex_edges_connected, is_edge_occupied, min_edge,
                             max_edge)
