def print_straight(T):
    """Funkcja do ładnego wypisywania tablicy."""
    # Zamień None na '#'
    ...
    # Wypośrodkuj kolumny i wypisz tablicę
...
def warrior(G, s, t):
    ...
    while not Q.empty():
        ...
        # Przeglądanie sąsiadów wierzchołka curr_v
        for neighbor, edge in G_sons[curr_v]:
            new_tiredness = tiredness + edge

            # Jeżeli zmęczenie nie przekracza limitu
            if new_tiredness < 17:
                if DQ[new_tiredness][neighbor] is None or DQ[new_tiredness][neighbor] > curr_dist + edge:
                    DQ[new_tiredness][neighbor] = curr_dist + edge
                    Q.put((curr_dist + edge, new_tiredness, neighbor))

                # Jeśli wojownik musi odpocząć (nowa odległość przy zerowym zmęczeniu)
                if DQ[0][neighbor] is None or DQ[0][neighbor] > curr_dist + edge + 8:
                    DQ[0][neighbor] = curr_dist + edge + 8
                    Q.put((curr_dist + edge + 8, 0, neighbor))

    # Wypisz tablicę DQ dla debugowania
    # print_straight(DQ)

    # Znalezienie minimalnej odległości do wierzchołka t (miasteczko docelowe)
    min_end = float('inf')
    for i in range(17):
...
s = 0
t = 6
print("Minimalny czas podróży:", warrior(G, s, t))
