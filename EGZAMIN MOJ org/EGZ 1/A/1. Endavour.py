def armstrong(B,G,s,t):
    ...
    for v_bike in range(all_v):
        if extract_ratings[v_bike] != None:
            l, m = extract_ratings[v_bike]
            distans = int((no_bike_dist_all[v_bike] + l * do_mety_z_rowerem[v_bike] / m)//1)
            min_with_bike = min(distans, min_with_bike)
