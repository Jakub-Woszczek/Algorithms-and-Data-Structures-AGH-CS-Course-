        ...
        while not Q.empty():
            ...
            for i in range(len(G_sons[curr_vertex])):
                ...
                if been_here[next_vertex] == False:
                    ...
                    # Teraz trza by zrobić relaxamcje


                    if timeings[next_vertex] > timeings[curr_vertex] + edge_weight:
                        timeings[next_vertex] = timeings[curr_vertex] + edge_weight

                        # Teraz sprawdzam czy dojście do następnego wierzchołka nie sprawi że przekroczy 16h
                        if curr_tiredness + edge_weight > 16:

                            # Poprawiam timeing następnego wierchołka ( drzenmnął się w curr_vertex ) ale hurka dukrka też musi być mniejszy :3
                            if timeings[next_vertex] > timeings[curr_vertex] + 8:
                                timeings[next_vertex] += 8
                            Q.put([timeings[next_vertex],next_vertex,0])
                            # print(f'spanko między wierzchołkami {curr_vertex} i {next_vertex}')
                        else:
                            # Tutaj poprostu dodaje do zmęczenia krawędź którą idzie
