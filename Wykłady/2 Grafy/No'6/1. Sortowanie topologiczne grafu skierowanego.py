...
def sortowanie_topologiczne(L):
    ...
    def DFS(v,T_ver): # v - index wieszchołka (nr - 1), T_ver tablica somsiadów

        # Tutaj robie warunek końcowy rekurencji jeżeli już tam nie byłem
        if T_ver[v] != None and len(T_ver[v]) == 0:
            T_ver[v] = None
            sorted_ver.insert(0,v+1)
            return

        # Tutaj idę do kolejnch, jeżeli istnieją
        if T_ver[v] != None:
            for v_next in T_ver[v]:
                if T_ver[v_next-1] != None:

                    DFS(v_next-1,T_ver)

        # Po przejściu trzeba dodać tego ostatniego bękarta
        T_ver[v] = None
        sorted_ver.insert(0,v+1)
        return

    sorted_ver = []
    num_of_ver, T_ver = num_of_ver_and_tablica_sasiedztwa(L)

    while warunek_czy_zostało_grafu(T_ver) == False:

        # Tutaj pętla będzie ropoczynała DFS-y, tyle ile potrzeba
        index_vertexu = 0
        while index_vertexu < len(T_ver): # Tutaj szukam jeszcze nie zaczętego wieszchołka który został
            # index_vertexu - index wieszchołka od którego można czacząć DFS
            if T_ver[index_vertexu] != None:
                DFS(index_vertexu,T_ver)
            index_vertexu += 1


    return sorted_ver

print(sortowanie_topologiczne(L_2))
